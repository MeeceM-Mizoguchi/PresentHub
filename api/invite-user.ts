...
function buildInviteHtml(name: string, url: string): string {
  return `<!DOCTYPE html>
<html lang="ja">
...
async function sendInviteEmail(resendApiKey: string, email: string, name: string, url: string): Promise<void> {
...
export default async function handler(req: any, res: any) {
  ...
  if (error) {
    // 既存ユーザーの場合: admin REST APIでユーザーを検索してパスワードリセットリンクを送信
    try {
      ...
      if (existingUser) {
        // パスワードリセットリンクを生成してResendで送信（新規招待と同じメール）
        ...
        if (resendApiKey && recoveryUrl) {
          await sendInviteEmail(resendApiKey, email, name, recoveryUrl).catch(err =>
            console.error('[invite-user] Resend existing-user error:', err)
          );
        }
  ...
  const inviteUrl: string | null = (data as any)?.properties?.action_link ?? null;

  if (resendApiKey && inviteUrl) {
    await sendInviteEmail(resendApiKey, email, name, inviteUrl).catch(err =>
